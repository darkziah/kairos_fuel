...
RegisterServerEvent("esx_fuel:pagamento")
AddEventHandler("esx_fuel:pagamento",function(price,galao,vehicle,fuel,fuel2,key)
	local source = source
	local output = {}
	local query = nil
	if key then
	...
	if price > 0 then
		...
		if galao or output.stock > amount then
			...
			if money >= price then
				...
				if galao then
					TriggerClientEvent('esx_fuel:galao',source)
					TriggerClientEvent("dlrms_notify",source,"success","Pagou <b>$"..price.." </b> pelo <b>Galão</b>.",8000)
				else
					...
					TriggerClientEvent("dlrms_notify",source,"success",amount.." litre için <b>$"..price.." </b> ödendi.",8000)
				end
			else
				TriggerClientEvent('esx_fuel:insuficiente',source,vehicle,fuel2)
				TriggerClientEvent("dlrms_notify",source,"error","Paranız yetersiz.",8000)
			end
		else
			TriggerClientEvent('esx_fuel:insuficiente',source,vehicle,fuel2)
			TriggerClientEvent("dlrms_notify",source,"error","Petrol Ofisinde yeteri kadar stock yok! (Max: "..output.stock.." litre alınabilir!).",8000)
		end
	end
end)

RegisterServerEvent('esx_fuel:LixeiroCB')
AddEventHandler('esx_fuel:LixeiroCB', function(key)
	local source = source
	if key then
		local sql = "SELECT stock,price FROM gas_station_business WHERE gas_station_id = @gas_station_id";
		...
		TriggerClientEvent('esx_fuel:LixeiroCB', source, output)
	else
		local output = {
			['price'] = Config.stock.default_price,
			['stock'] = Config.stock.default_stock,
